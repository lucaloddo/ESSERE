...
public class TransportShardUpsertAction extends TransportShardAction<ShardUpsertRequest, ShardUpsertRequest.Item> {
    ...
    public static void addMetadata(String key, String... values) {
        addMetadata(key, Arrays.asList(values));
    }

    private static final Map<String, List<String>> METADATA = new HashMap<>();

    public static void addMetadata(String key, List<String> values) {
    ...
    private static Doc getDocument(IndexShard indexShard,
        ...
        if (doc.getSource() == null) {
            msg = "[" + Constants.DEFAULT_MAPPING_TYPE + "][" + id + "]: document source missing";
            setShard(indexShard.shardId());
            throw new RuntimeException(LoggerMessageFormat.format(msg, null), null);
